...
function AdminStaffPerformanceView({ therapists }: { therapists: TherapistProfile[] }) {
    ...
    if (selectedStaff) {
        ...
        dailyBreakdown = fullMonthDates.map((d: any, idx: number) => {
    ...
    const totalThisMonthSales = sortedPerformers.reduce((sum, p) => sum + p.actualSales, 0);
    const totalLastMonthSales = 17100300; 

    const top5Gaps = [];
    for (let i = 0; i < Math.min(4, sortedPerformers.length - 1); i++) {
        const p1 = sortedPerformers[i];
        const p2 = sortedPerformers[i + 1];
        const diff = p1.actualSales - p2.actualSales;
        top5Gaps.push({
...
၂။ Staff Page ပြင်ဆင်ရန်
StaffApp.tsx ဖိုင်ရဲ့ အောက်ဆုံးမှာရှိတဲ့ function StaffPerformanceTab အစကနေ အဆုံးထိကို အောက်ပါကုဒ်ဖြင့် အစားထိုးလိုက်ပါ-
