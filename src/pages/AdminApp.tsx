...
function AdminManagementList() {
  ...
  const handleAddAdmin = async (e: React.FormEvent) => {
    ...
    try { await createUserWithEmailAndPassword(secondaryAuth, `${newAdmin.username.toLowerCase()}@shangrila.com`, newAdmin.password); await addDoc(collection(db, 'admins'), { username: encryptText(newAdmin.username), password: encryptText(newAdmin.password), role: newAdmin.role, permissions: newAdmin.role === 'super_admin' ? ['bookings', 'reports', 'users', 'points', 'admins', 'settings'] : newAdmin.permissions }); setNewAdmin({ username: '', password: '', role: 'super_admin', permissions: ['bookings', 'reports', 'users', 'points', 'penalties', 'settings']}); fetchAdmins(); } catch (e) { alert('Error adding admin. It may already exist in Auth.'); }
  };
