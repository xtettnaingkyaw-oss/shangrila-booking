...
import { ClipboardList, Users, RotateCw, Save, Coffee, Droplets, Bed, ChefHat, Bath, CheckCircle, Info, Edit, Trash2, PlusCircle, X } from 'lucide-react';
import { TherapistProfile } from '../shared';

const DEFAULT_TASKS = [
    { id: 'reception', title: '၁။ ဧည့်ကြိုနှင့် ဧည့်ခန်းစောင့်တာဝန်', iconName: 'Coffee', color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200', desc: ['ဆိုင်၏ ပထမဆုံး မျက်နှာစာဖြစ်သောကြောင့် အမြဲတမ်း ဖော်ရွေပျူငှာပြီး သန့်ရှင်းသပ်ရပ်နေရန်။', 'ကြိုဆိုခြင်း - ဧည့်သည်များကို အပြုံးဖြင့် နွေးထွေးစွာ ကြိုဆိုရန်။', 'ဧည့်ခန်းတာဝန်ကျသူ တစ်ယောက်သည် အမြဲတမ်း Stand By ရှိရမည်။', 'ဧည့်ခန်းသန့်ရှင်းရေး - ဖုန်ကင်းစင်အောင် နေ့စဉ် သန့်ရှင်းရေးလုပ်ရန်။', 'ဆိုင်ရှေ့မျက်နှာစာ - တံမြက်စည်းလှည်း၊ ရေဆေးချရန်။'] },
    { id: 'water_garbage', title: '၂။ ရေစောင့်တင်ရန်နှင့် အမှိုက်ပစ်တာဝန်', iconName: 'Droplets', color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200', desc: ['ရေရရှိမှု စီမံခြင်း - ရေတိုင်ကီများ ပုံမှန်စစ်ဆေးပြီး ရေမော်တာကို အချိန်မှန် ဖွင့်/ပိတ် လုပ်ရန်။', 'အမှိုက်စွန့်ပစ်ခြင်း - အမှိုက်ပုံးကြီးအား နေ့စဉ် စွန့်ပစ်ရန်။'] },
    { id: 'laundry_rooms', title: '၃။ တဘက်/အခင်းလျှော်နှင့် အခန်းသန့်ရှင်းရေး', iconName: 'Bed', color: 'text-purple-600', bg: 'bg-purple-50', border: 'border-purple-200', desc: ['လျှော်ဖွပ်ခြင်း - တဘက်နှင့် အခင်းများကို သန့်စင်အောင် လျှော်ဖွပ်ရန်။', 'အခန်းပြင်ဆင်ခြင်း - မနက်ဆိုင်မဖွင့်မီ အခန်းများကို ရှင်းလင်းရန်။'] },
    { id: 'kitchen_cooking', title: '၄။ မီးဖိုချောင်နှင့် ထမင်းချက်တာဝန်', iconName: 'ChefHat', color: 'text-red-600', bg: 'bg-red-50', border: 'border-red-200', desc: ['ချက်ပြုတ်ခြင်း - ကျန်းမာရေးနှင့် ညီညွတ်သော အစားအသောက်များကို အချိန်မီ ပြင်ဆင်ချက်ပြုတ်ရန်။', 'မီးဖိုချောင်သန့်ရှင်းရေး - ချက်ပြုတ်ပြီးတိုင်း ဆေးကြောရန်။'] },
    { id: 'bathroom_toilet', title: '၅။ ရေချိုးခန်းနှင့် အိမ်သာ သန့်ရှင်းရေး', iconName: 'Bath', color: 'text-teal-600', bg: 'bg-teal-50', border: 'border-teal-200', desc: ['အမြဲတမ်း ခြောက်သွေ့စေရန် - မနက်တိုင်း ရေချိုးခန်းနှင့် အိမ်သာကြမ်းပြင်များကို ခြောက်သွေ့စေရန်။', 'တိုက်ချွတ်သန့်စင်ခြင်း - နေ့စဉ် စနစ်တကျ တိုက်ချွတ်ဆေးကြောရန်။'] }
];
...
export function DutyRosterSystem({ therapists }: { therapists: TherapistProfile[] }) {
    ...
    // 🌟 ဤနေရာသည် ဝန်ထမ်းတစ်ဦးတည်းကို တာဝန် ၂ ခုထက်ပိုပေးနိုင်အောင် ပြင်ဆင်ထားသောနေရာဖြစ်သည် 🌟
    ...
    if (loading) return <div className="p-10 text-center font-bold animate-pulse">Loading Duty Roster...</div>;
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 relative">
                ...
                <div className="fixed inset-0 z-[99] bg-black/60 flex items-center justify-center p-4">
                    <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col animate-slide-up">
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Duty Title</label>
                        ...
                        <div className="pt-4 border-t mt-4 flex gap-3">
                            <button onClick={() => setEditingTask(null)} className="flex-1 py-3 bg-gray-100 font-bold rounded-xl text-gray-600">Cancel</button>
                            <button onClick={saveEditedTask} className="flex-1 py-3 bg-[#123524] text-white font-bold rounded-xl">Done</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 border-b border-gray-100 pb-4 gap-4">
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">လူတစ်ဦးတည်းကို တာဝန် ၂ ခုပေးနိုင်ပါသည်။ ခေါင်းစဉ်ဘေးရှိ Edit ခလုတ်နှိပ်၍ စာသားများကိုပြင်ပါ။</p>
                </div>
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    <button onClick={handleAutoRotate} className="px-4 py-2 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-xs font-bold flex items-center shadow-sm">
                        <RotateCw className="w-4 h-4 mr-1.5" /> Auto Rotate
                    </button>
                    <button onClick={handleSaveRoster} disabled={saving} className="px-4 py-2 bg-[#123524] text-[#D4AF37] border border-[#1a4a32] rounded-lg text-xs font-bold flex items-center shadow-md disabled:opacity-50">
                        <Save className="w-4 h-4 mr-1.5" /> Save & Update
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {tasks.map(task => {
                    ...
                    return (
                        <div key={task.id} className={`rounded-xl border ${task.border} shadow-sm flex flex-col bg-white`}>
                            <div className={`${task.bg} p-4 border-b ${task.border} flex justify-between items-start`}>
                                ...
                                <div className="flex gap-1">
                                    <button onClick={() => setEditingTask(JSON.parse(JSON.stringify(task)))} className="p-1.5 bg-white/60 hover:bg-white rounded-md text-gray-500 shadow-sm"><Edit className="w-3.5 h-3.5"/></button>
                                </div>
                            </div>

                            {expandedTask === task.id && (
                                <div className="p-4 bg-gray-50 border-b border-gray-100 text-xs text-gray-600 font-semibold animate-fade-in">
                                    <ul className="space-y-1">{task.desc.map((d:string, i:number) => <li key={i}>• {d}</li>)}</ul>
                                </div>
                            )}
                                    ...
                                    {assignedStaffIds.map(staffId => {
                                        ...
                                        return (
                                            <div key={staffId} className="bg-gray-100 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center shadow-sm border border-gray-200">
                                                ...
                                                <button onClick={() => handleRemoveStaff(task.id, staffId)} className="ml-2 text-red-400 hover:text-red-600"><RotateCw className="w-2.5 h-2.5 rotate-45"/></button>
                                            </div>
                                        );
                                    })}
                                </div>
                                <select className="w-full p-2 bg-gray-50 border rounded text-xs font-bold outline-none" onChange={(e) => { handleAssignStaff(task.id, e.target.value); e.target.value = ''; }} defaultValue="">
