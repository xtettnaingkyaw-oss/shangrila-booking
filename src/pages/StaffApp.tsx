...
import { collection, query, onSnapshot, doc, updateDoc, addDoc, orderBy, getDocs, getDoc, setDoc } from 'firebase/firestore';
import { db } from '../firebase';
import { LogOut, User, Clock, CheckCircle, ChevronLeft, CalendarPlus, History, Coffee, Sparkles, Trash2, Calendar, ShieldAlert, KeyRound, ChevronDown, Droplets, Home, Activity, Crown, Percent, Copy, AlertCircle } from 'lucide-react';
import { THEME, AppData, Booking, OutPass, TherapistProfile, MenuItem, UserProfile, formatPrice } from '../shared';
  ...
  const generateTimeline = (therapistName: string) => {
      ...
      tBookings.forEach(b => {
           ...
           if (coveredSlots.length > 0) {
               ...
               if (firstIdx > 0 && !coveredMap.has(ALL_TIME_SLOTS[firstIdx - 1])) {
                   coveredMap.set(ALL_TIME_SLOTS[firstIdx - 1], { service: 'နားချိန် (Rest)', status: 'Buffer' });
               }
               const lastIdx = ALL_TIME_SLOTS.indexOf(coveredSlots[coveredSlots.length - 1]);
               if (lastIdx !== -1 && lastIdx < ALL_TIME_SLOTS.length - 1 && !coveredMap.has(ALL_TIME_SLOTS[lastIdx + 1])) {
                   coveredMap.set(ALL_TIME_SLOTS[lastIdx + 1], { service: 'နားချိန် (Rest)', status: 'Buffer' });
               }
           }
      });
