...
import { User, ShieldAlert, KeyRound, ChevronDown, LogOut, ChevronLeft, CalendarPlus, History, Coffee, CheckCircle, Calendar, Clock, Trash2, Droplets } from 'lucide-react';
import { THEME, AppData, Booking, OutPass, TherapistProfile } from '../shared';
import { CustomerBookingWizard } from './CustomerApp';

// ==========================================
// LOCAL HELPERS (Retained exactly from your Main Code)
// ==========================================
...
function StaffOutPassTab({ appData, loggedInStaff }: { appData: AppData, loggedInStaff: TherapistProfile }) {
   ...
   const handleGoOut = async () => {
       ...
       navigator.geolocation.getCurrentPosition(async (pos) => {
           ...
           // ၅၀ မီတာ ပြောင်းလဲသတ်မှတ်ထားပါသည်
           if (dist > 50) { 
           ...
           await addDoc(collection(db, 'outpasses'), {
               therapist: loggedInStaff.name, date: todayStr, outTimeMillis: now, expectedInTimeMillis: now + 30 * 60 * 1000, status: 'out', reason: reason.trim()
           });
   ...
   const handleReturn = async () => {
       ...
       navigator.geolocation.getCurrentPosition(async (pos) => {
           ...
           await updateDoc(doc(db, 'outpasses', myActivePass.id), {
               status: 'returned', inTimeMillis: now, overtimeSeconds: Math.floor(overtimeMillis / 1000)
           });
           setLocating(false);
       }, (err) => {
           setLocError("Location (GPS) ဖွင့်ပေးရန် လိုအပ်ပါသည်။"); setLocating(false);
