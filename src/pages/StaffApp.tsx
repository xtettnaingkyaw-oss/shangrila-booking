   ...
   return (
       <div className="bg-white p-4 sm:p-8 rounded-2xl shadow-sm border border-gray-100 animate-fade-in relative">
           ...
           <div className="relative mb-6">
               <div className="flex space-x-1 sm:space-x-2 bg-gray-50 p-1.5 rounded-xl border border-gray-100 relative">
                   <button onClick={() => { setStaffTab('service'); setShowMoreTabs(false); }} className={`flex-1 px-3 py-2 text-[10px] sm:text-xs font-bold rounded-lg transition ${staffTab === 'service' ? 'bg-white shadow text-[#123524]' : 'text-gray-500 hover:bg-gray-100'}`}>Service</button>
               ...
               {/* 🌟 Dropdown Menu 🌟 */}
               {showMoreTabs && (
                   <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden animate-slide-up flex flex-col p-1.5">
                       <button onClick={() => { setStaffTab('history'); setShowMoreTabs(false); }} className={`px-4 py-3 text-xs font-bold text-left rounded-lg transition flex items-center ${staffTab === 'history' ? 'bg-gray-50 text-[#123524]' : 'text-gray-600 hover:bg-gray-50'}`}>
                           <History className="w-4 h-4 mr-2 text-gray-400" /> History
                       </button>
                       
                       <button onClick={() => { setStaffTab('financials'); setShowMoreTabs(false); }} className={`px-4 py-3 text-xs font-bold text-left rounded-lg transition flex items-center justify-between ${staffTab === 'financials' ? 'bg-purple-50 text-purple-700' : 'text-gray-600 hover:bg-gray-50'}`}>
                           <span className="flex items-center"><Banknote className="w-4 h-4 mr-2 text-purple-400" /> Fine & Loan</span>
                           {unpaidPenaltyCount > 0 && <span className="bg-red-500 text-white text-[9px] w-4 h-4 flex items-center justify-center rounded-full shadow-md animate-pulse">{unpaidPenaltyCount}</span>}
                       </button>
                       
                       <button onClick={() => { handleGoToRoster(); setShowMoreTabs(false); }} className={`px-4 py-3 text-xs font-bold text-left rounded-lg transition flex items-center justify-between ${staffTab === 'roster' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50'}`}>
