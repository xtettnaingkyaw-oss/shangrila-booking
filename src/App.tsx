...
const DEFAULT_BRANDING: AppBranding = {
  logoUrl: '', name: "The Shangri-La Men's Retreat", address: "33th(B) St, Between 65th & 65th(A) Sts, Mandalay",
...
function App() {
  ...
  useEffect(() => {
    ...
    if (appData?.branding?.logoUrl) {
      ...
      const manifest = { name: appName, short_name: appName, start_url: "/", display: "standalone", background_color: "#ffffff", theme_color: THEME.primary, icons: [{ src: appData.branding.logoUrl, sizes: "192x192", type: "image/png" }, { src: appData.branding.logoUrl, sizes: "512x512", type: "image/png" }] };
  ...
  // Use useCallback for settings update to prevent unnecessary re-renders of Admin component
  const handleSettingsUpdated = useCallback((newData: AppData) => {
    setAppData(newData);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans flex flex-col">
        ...
        <div className="fixed inset-0 z-[200] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden animate-fade-in shadow-2xl">
             ...
             <div className="p-5 max-h-[75vh] overflow-y-auto space-y-4">
                ...
                <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-sm">
                   <p className="text-xs font-bold mb-2">၂။ "Add to Home Screen" ကို ရွေးချယ်ပါ။</p>
                </div>
                <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-sm">
                   <p className="text-xs font-bold mb-2">၃။ "Add" ကို နှိပ်ပါ။ ဖုန်း Screen တွင် App အဖြစ် ရောက်ရှိသွားပါမည်။</p>
                </div>
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto p-4 py-6">
        {appMode === 'admin' ? (
          loggedInAdmin ? <AdminDashboard appData={appData} onSettingsUpdated={handleSettingsUpdated} /> : <AdminLogin onLogin={(user) => { setLoggedInAdmin(user); sessionStorage.setItem('shangrila_admin', user); }} />
        ) : appMode === 'staff' ? (
...
// ==========================================
// CUSTOMER BOOKING WIZARD (Memoized)
// ==========================================
...
// ==========================================
// HISTORY, PROFILE & AUTH COMPONENTS
// ==========================================
...
const AdminDashboard = memo(({ appData, onSettingsUpdated }: { appData: AppData, onSettingsUpdated: (data: AppData) => void }) => {
  ...
  // Memoized filtered bookings to prevent unneccessary re-renders
